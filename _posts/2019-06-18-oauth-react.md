...
1. You will not be able to submit this app for verification for OAuth. This means when the OAuth window pops up for the user, it will always alert the user that this is not a verified app. This is because it is a demo app and Google does not verify demos.

1. After completing this tutorial, if you'd like to host it via a live website (if you've verified `https://{username}.github.io` URL with Google) you can follow these steps to [launch a `create-react-app` to GitHub Pages](https://github.com/gitname/react-gh-pages).
